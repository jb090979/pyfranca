...
class TestReferences(BaseTestCase):
    ...
    def test_ambiguous_names(self):
        ...
        my_struct = self.processor.packages["P"].interfaces["I"].structs["ABC"]
        self.assertEqual(my_struct.name, "ABC")

        my_method = self.processor.packages["P"].interfaces["I"].methods["ABC"]
        self.assertEqual(my_method.name, "ABC")

    def test_ambiguous_names_references(self):
        fspec = self.tmp_fidl("P.fidl", """
                        package P
                        typeCollection T {
                            version { major 1 minor 0 }
        ...
        my_struct = self.processor.packages["P"].typecollections["T"].structs["ABC"]
        self.assertEqual(my_struct.name, "ABC")
